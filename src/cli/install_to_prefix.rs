...
pub async fn execute(args: Args) -> miette::Result<()> {
    ...
    let (python_status, all_conda_packages) = if !conda_packages.is_empty() {
        let python_status = install_conda_packages(&workspace, &prefix, conda_packages.clone(), platform).await?;
        ...
        if !pypi_packages.is_empty() {
            ...
            if !python_packages.is_empty() {
                ...
                let python_status = install_conda_packages(&workspace, &prefix, python_packages.clone(), platform).await?;
