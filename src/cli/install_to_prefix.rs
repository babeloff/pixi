...
use crate::environment::{update_prefix_pypi, CondaPrefixUpdater, PythonStatus};
use crate::lock_file::PypiRecord;
use crate::prefix::Prefix;
use crate::workspace::{get_activated_environment_variables, grouped_environment::GroupedEnvironmentName};
...
pub async fn execute(args: Args) -> miette::Result<()> {
    ...
    let lock_file = tokio::task::spawn_blocking(move || {
        LockFile::from_path(&lock_file_path)
            ...
            .with_context(|| format!("Failed to load lock file from `{}`", lock_file_path.display()))
    })
    .await
    ...
    let locked_environment = lock_file
        ...
        .ok_or_else(|| miette::miette!("Environment '{}' not found in lock file", args.environment))?;
    ...
    std::fs::create_dir_all(prefix.root())
        .into_diagnostic()
        .with_context(|| format!("Failed to create prefix directory: {}", prefix.root().display()))?;
    ...
    let python_status = if !conda_packages.is_empty() {
        install_conda_packages(&workspace, &prefix, conda_packages, platform).await?
    } else {
        // If no conda packages, we still need to check for Python
...
async fn install_conda_packages(
    ...
    let build_context = crate::build::BuildContext::from_workspace(workspace, command_dispatcher.clone())?;
...
async fn install_pypi_packages(
    ...
    update_prefix_pypi(
        ...
        prefix,
        platform,
        &[], // No conda records needed for PyPI-only installation
        &pypi_packages,
        python_status,
        &system_requirements,
        &uv_context,
